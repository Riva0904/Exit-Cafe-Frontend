...
import { myNotificationsApi } from '@/api/notifications';
...
export function CustomerNotificationBell() {
  ...
  const { data: unreadCount } = useQuery({
    ...
    queryFn: myNotificationsApi.getUnreadCount,
    refetchInterval: 15_000,
  });

  const { data: notifications } = useQuery({
    queryKey: ['my-notifications', 'list'],
    queryFn: myNotificationsApi.getAll,
    enabled: open,
  });

  const markOneRead = useMutation({
    mutationFn: myNotificationsApi.markAsRead,
                  ...
                  notifications.map((n) => (
                    <button
                      ...
                      onClick={() => {
                        ...
                        if (n.type === 'RateOrder' && n.relatedEntityId) {
                          navigate(`/orders/${n.relatedEntityId}/confirmation`);
                        }
                        setOpen(false);
                      }}
